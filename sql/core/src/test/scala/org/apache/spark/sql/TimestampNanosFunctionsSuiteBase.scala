...
abstract class TimestampNanosFunctionsSuiteBase extends SharedSparkSession {
  ...
  // ===== collect_list / collect_set / mode over nanosecond-precision timestamps (SPARK-56822) =====
  // `CollectList` (ArrayBuffer buffer), `CollectSet` (HashSet dedup) and `Mode` (OpenHashMap key
  // counting) are all type-agnostic: their buffers hold the physical `TimestampNanosVal`, whose
  // `equals`/`hashCode`/`compareTo` cover the full `(epochMicros, nanosWithinMicro)` pair. So set
  // dedup and frequency counting distinguish sub-microsecond-distinct values, and the result
  // element/child type is exactly `child.dataType` -- the input precision and family (NTZ/LTZ) are
  // preserved with no truncation to micros. These end-to-end tests lock that in; no change to the
  // aggregates was needed. The collection order of `collect_list`/`collect_set` is not deterministic
  // after aggregation, so their contents are compared as sets.
